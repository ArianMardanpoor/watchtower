...
import { getLiveSubdomains } from '@/api/live';
...
export default function LiveSubdomains() {
  ...
  const handleExport = async () => {
    try {
      const result = await downloadExport('/export/lives', filters);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              {!isLoading && responseData?.total ? `Showing ${(page - 1) * perPage + 1} to ${Math.min(page * perPage, responseData.total)} of ${responseData.total} live subdomains` : 'Monitor active subdomains'}
            </p>
          </div>
